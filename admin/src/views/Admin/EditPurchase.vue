...
const fetchPurchase = async () => {
  ...
  try {
    // 1. Fetch header
    const resHeader = await authFetch(`/api/purchases/${purchaseId}`);
    if (!resHeader.ok) throw new Error('Error al obtener los datos de la compra.');
    const dataHeader = await resHeader.json();
    
    formData.ticketNumber = dataHeader.ticketNumber;
    ...
    if (resItems.ok) {
      ...
      formData.items = dataItems.map((item: any) => ({
        cantidad: Number(item.quantity),
        medida: item.unit,
        producto: item.productName,
        costType: item.costType || 'Directo',
    ...
    apiError.value = e.message || 'Error al cargar los datos de la compra';
  } finally {
    loading.value = false;
  }
};
...
const saveCompra = async () => {
  ...
  if (formData.items.length === 0) {
    apiError.value = 'Debes añadir al menos un concepto en el detalle de la compra.';
    return;
  }
  ...
  try {
    ...
    const responseHeader = await authFetch(`/api/purchases/${purchaseId}`, {
      method: 'PUT',
      headers: {
    ...
    if (!responseHeader.ok) {
      const error = await responseHeader.json();
      throw new Error(error.error || 'Error al actualizar cabecera');
    }

    // 2. Overwrite purchase items breakdown
    const responseItems = await authFetch(`/api/purchases/${purchaseId}/items`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!responseItems.ok) {
      ...
      throw new Error(error.error || 'Error al actualizar conceptos');
    }
    
    router.push('/purchases');
  } catch (e: any) {
    apiError.value = e.message || 'Error al actualizar la compra';
  } finally {
    saving.value = false;
  }
};
