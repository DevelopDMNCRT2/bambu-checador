# Bambú Asistente 🎋 - Documentación Técnica y Funcional

Sistema seguro de asistencia, control de horarios y administración de compras para el personal y gerencia de **Bambú**. Este repositorio contiene una arquitectura dividida en dos módulos principales: un panel de administración/nómina para gerencia y un checador público seguro con geolocalización.
...
1. **/admin** (Frontend Administrativo & Checador)
   ...
   - **Propósito:** Contiene tanto el panel administrativo privado (gestión de empleados, turnos, nómina, compras por XML/manuales, reportes y descargas en CSV) como la vista pública del **/checador** (donde los empleados marcan sus entradas y salidas).
2. **/server** (Backend & API)
   - **Tecnologías:** Node.js, Express, PostgreSQL (alojado en Neon).
   - **Propósito:** API que gestiona la base de datos de empleados, registros de asistencia, geolocalización de checado (máximo 200m del restaurante), generación de tokens de enlace para vincular dispositivos, procesamiento de facturas XML y el planificador automático del corte de nómina.
...
## 📝 Historial de Funcionalidades y Contexto Actual (Julio 2026)
...
- **Simplificación del Sidebar:**
  - Se eliminaron todos los módulos ajenos a este proyecto (Caja, POS, Ventas, etc.). El sidebar ahora contiene exclusivamente accesos directos y limpios a **Empleados**, **Nómina** y **Compras y Gastos**.
